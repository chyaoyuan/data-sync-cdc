...
class GleUserConfig:
    ...
    # 去掉_为真参数
    env_field_default_list = ["_https://fsgtest.gllue.net", "_824531e8cad2a287", "_api@fsg.com.cn"]
    env_var = {}
...
class SyncConfig:
    env_field_name_list = ["EntityName", "Recent", "Unit", "OrderBy", "SyncModel", "TimeFieldName"]
    env_field_default_list = ["jobOrder", "3", "day", "-id", "Recent", "lastUpdateDate__day_range"]
    env_var = {}
    for _env_name, _default in zip(env_field_name_list, env_field_default_list):
        env_var[_env_name[0].lower()+_env_name[1:]] = os.getenv(_env_name, _default)
