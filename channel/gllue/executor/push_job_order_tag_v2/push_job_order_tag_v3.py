                ...
                try:
                    ...
                    if gle_data != storage_data and gle_data:
                        ...
                        duty_tag_info_list = await tip_tag_app.expand_flatten(
                            {"texts": [jmespath.search("jobTitle", job_order)], "output_category": "position3", "top_k": 1, "rerank": True})
                        try:
                            ...
                            logger.error(f"GBT ERROR PASS ->{e}")
                            continue
                        logger.info(f"GBT的维度结果->{extract_info_dict}")
                        ...
                        try:
                            ...
                            logger.info(f"GBT ERROR {e}")

                        expand_tag_list = await tip_tag_app.expand_flatten({
                        ...
                        need_put_entity = {}
                        for k, v in job_order.items():
                            if not v and new_gllue_body.get(v):
                                need_put_entity[k] = new_gllue_body.get(v)
                        logger.info(f"finally entity->{need_put_entity}")
                        if not need_put_entity:
                            ...
                            res = await gle_push_app.job_order_app.update_job_order_by_id(job_order_id=job_order["id"],overwrite_info=need_put_entity)
                            if res.get("status"):
                                _data = await tip_app.source_entity_storage_app.put_entity(
                                    {"tenant": tip_config.tenant_alias,
                                     "source_entity_type": "JobOrder",
