...
class GleEntity(GleSchema):
    ...
    async def get_attachment(self, entity, field_name_list, attachments):
    ...
    async def _get_candidate_info(self, page: int, field_name_list: str, check: bool = False, overwrite_gql: Optional[str] = None):
        logger.info(overwrite_gql if overwrite_gql else self.sync_config.gql)
        ...
        for index, candidate in enumerate(res["result"][self.entity]):
            ...
            if attachments and self.sync_config.syncAttachment:
                await self.get_attachment(entity, field_name_list, attachments)
    ...
    async def initialize_field(self, add_field_list: Optional[list] = None, add_child_field_list: Optional[list] = None):
        ...
        logger.error(add_child_field_list)
