...
async def _asy_get_candidate_info(candidate_id: str, headers: dict,session: ClientSession,source_id,semaphore):
    async with semaphore:
        ...
        if status_code != 200:
            quit()
        async with aiofiles.open("/Users/chenjiabin/Project/data-sync-cdc/channel/linkedin/candidate_id/success_candidate.jsonl","a") as f:
            await f.write(json.dumps(body, ensure_ascii=False) + '\n')
        async with aiofiles.open("/Users/chenjiabin/Project/data-sync-cdc/channel/linkedin/candidate_id/success_candidate_id.jsonl", "a") as f:
            await f.write(json.dumps({"candidateId": source_id}, ensure_ascii=False)+'\n')
        print(candidate_id)

async def asy_get_candidate_info(candidate_id: str, headers: dict):
    async with aiohttp.ClientSession() as session:
        await _asy_get_candidate_info(candidate_id,headers,session)
...
if __name__ == '__main__':
    ...
    asyncio.run(asy_get_candidate_info("AEMAAAESxg4BMVomF6OvFHeR_KcuO7umYry4Gpw",_headers))
